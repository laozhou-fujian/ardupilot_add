...
// set zoom specified as a rate or percentage
bool AP_Mount::set_thermal_zoom(uint8_t instance, ZoomType zoom_type, float zoom_value)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
    }
    return backend->set_thermal_zoom(zoom_type, zoom_value);
}

// set zoom specified as a rate or percentage
bool AP_Mount::set_pseudo_color(uint8_t instance, float pcolor_value)
{
    auto *backend = get_instance(instance);
    if (backend == nullptr) {
        return false;
    }
    return backend->set_pseudo_color(pcolor_value);
}
